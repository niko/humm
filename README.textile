...
Yet another middleware for geoip lookup with Maxminds geoip database.

You can find the other one here:

* http://coderack.org/users/hosiawak/middlewares/36-geoip-country
...
Rack::MMGeoip uses the geoip gem. In my tests this lib did about 17_000 lookups in about 17 seconds, so you probably lose one ms per request when using this middleware.
